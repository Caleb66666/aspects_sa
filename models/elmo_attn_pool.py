# @File: albert_elmo_attn_pool
# @Author : Caleb
# @Email: VanderLancer@gmail.com
# @time: 2020/1/30 13:55:24
...
class SimpleElmo(nn.Module):
    ...
    def forward(self, embed_seq, seq_mask):
        lstm_outputs = self.elmo_lstm(embed_seq, seq_mask)
        output_tensors = [torch.cat([embed_seq, embed_seq], dim=-1) * seq_mask.float().unsqueeze(-1)]
        for layer_activations in torch.chunk(lstm_outputs, lstm_outputs.size(0), dim=0):
            output_tensors.append(layer_activations.squeeze(0))
        # batch_size, max_seq, input_size * 2
        return self.scalar_mix(output_tensors, seq_mask)
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        # 简化版的elmo模型作为特征抽取
        self.encoder = SimpleElmo(config.embed_dim, config.cell_size, config.num_layers, config.do_layer_norm)

        self.units, self.criterion_list = nn.ModuleList(), list()
        for _ in range(self.num_labels):
            unit = nn.Sequential(
                AttnPool(config.embed_dim * 2),  # bach_size, encode_hidden * 2
